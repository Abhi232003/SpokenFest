...
class Common_model extends CI_Model {
	...
	public function getRecords($tbl_name, $condition = FALSE, $select = FALSE, $order_by = FALSE, $start = FALSE, $limit = FALSE) {
	 ...
	 public function getSearchFilterRecords($fromDate,$toDate,$enquiryStatusFilter){
   	...
   	public function getyearLineup($yearId){
	    $this->db->select('LY.yearID,LY.year,LY.yearSort,LY.dateAdded,LY.status,L.lineupID,L.sortOrder,L.lineupYear,L.lineupHeading,L.seoUri,L.lineupThumbImage,L.lineupDetailImage,L.lineupDescription,L.dateAdded,L.dateModified,L.isActive,L.status');
	    $this->db->from("fx_lineup AS L");
	    $this->db->join('fx_years AS LY', 'LY.yearID=L.lineupYear', 'left');
	    $this->db->where('LY.status', 1);
	    $this->db->where('LY.yearID', $yearId);
	    $this->db->order_by('L.sortOrder', 'asc');
	    // $this->db->order_by('L.lineupID', 'asc');
...
// 		$this->db->where('isActive',1);
    ...
    public function LineupList(){
	    $this->db->select('LY.yearID,LY.year,LY.yearSort,LY.dateAdded,LY.status,L.lineupID,L.lineupYear,L.lineupHeading,L.seoUri,L.lineupThumbImage,L.lineupDetailImage,L.lineupDescription,L.dateAdded,L.dateModified,L.isActive,L.status');
	    $this->db->from("fx_lineup AS L");
	    $this->db->join('fx_years AS LY', 'LY.yearID=L.lineupYear', 'left');
	    $this->db->where('LY.status', 1);
	    $this->db->order_by('L.lineupID', 'asc');
