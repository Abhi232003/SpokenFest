...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        ...
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="<?= base_url('admin/news/add_edit'); ?>"><button class="btn btn-primary-rgba"><i class="feather icon-plus mr-2"></i>Add New news</button></a>
            </div>                        
        </div>
    </div>          
</div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable({
    "processing": true,
    "serverSide": false,
    "ajax": "<?=base_url('admin/news/datatable_json')?>",
    "order": [[0,'desc']],
    "columnDefs": [
    { "targets": 0, "name": "newsID", 'searchable':true, 'orderable':true,'width':'50px'},
    { "targets": 1, "name": "newsLink", 'searchable':true, 'orderable':true,'width':'100px'},
    { "targets": 2, "name": "newsThumbImage", 'searchable':true, 'orderable':true,'width':'100px'},
...
S
    $.post('<?=base_url("admin/news/news_delete")?>',
    {
      '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
      id : $(this).data('id'),
      status : $(this).is(':checked') == true?1:0
